...
qt.set_position(janela.width - qt.width - 20, janela.height - qt.height - 20)

qtred = Sprite("imagem/quitred.png")
qtred.x = janela.width/2 - qtred.width/2.3
qtred.y = janela.height/1.25
qtred.set_position(janela.width - qtred.width - 20, janela.height - qtred.height - 20)
...
def tela_ranking(point):
    ...
    while True:
        ...
        for i in range(len(player)):
            if i == 5:
                break
            janela.draw_text(player[i], janela.width / 2 - 140, 50 + 80 * (i + 1), size=55, color=(255, 255, 255))
